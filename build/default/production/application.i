...
timer1_config_t Timer1Obj={.TIMER1_HandlerInterrupt=Timer1InterruptHandler,.preloaded_value=45536,
                           ...
                           .timer1_mode_wr_reg=1,
                           .timer1_pre_scaler_val=0,
...
int main(void) {
    ...
    Timer1_Init(&Timer1Obj);
    seven_segment_initialize(&SegObj);
    led_turn_on(&LedGreen);
    led_turn_off(&LedYellow);
    led_turn_off(&LedRed);
    ADCON1bits.PCFG=0x0F;
    while(1){

        seven_segment_write_number(&SegObj,u8ValueOfSeg);
      if(ValueOfSecond==9){
        led_turn_off(&LedGreen);
        led_turn_off(&LedRed);
        led_turn_on(&LedYellow);
        u8ValueOfSeg=14 -9;
        }
    if(ValueOfSecond==14){
        led_turn_off(&LedGreen);
